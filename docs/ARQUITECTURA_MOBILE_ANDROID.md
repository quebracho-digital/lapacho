...
| **Syncthing / SuperSync as clipboard store** | **Not as vault** | Fine as *optional* way to move opaque files someday; wrong UX and ACL for per-item secrets. SuperSync on SER5 stays file sync, not Lapacho history. |
| **Matrix E2EE as transport** | **Optional adapter later** | Attractive if we already live on Tuwunel; still our envelopes inside room messages. Not required for v1. |
...
| **B. Direct / LAN boost** | Same LAN or WireGuard (`10.13.13.0/24`) | Prefer direct HTTPS/QUIC or WebSocket between peers if discovery succeeds; lower latency, less load on SER5. |
...
2. First transport adapter: **self-hosted relay** on SER5 (small service under QuebrachOS / Caddy, tunnel hostname e.g. `lapacho-sync.quebrachos.digital` or LAN-only + WG).  
...
- **Personal / PYME:** SER5 (or any tiny VPS the user controls).  
...
- Aligning with existing SER5 SSO (same as Matrix, Kuma, etc.).
...
- Syncthing/SuperSync as the clipboard database.  
...
| D14 | Hybrid topology: SER5 relay + optional LAN/WG direct | NAT reality; QuebrachOS-aligned; §5.6 / S2–S3 |
...
10. Whether a Matrix adapter is worth it after HTTP relay exists (Tuwunel already on SER5).
